...
func allocateBuffer(size int) (b []byte, err error) {
	...
	if mmap1Err != nil {
		err = fmt.Errorf("bytebuffers.Buffer: first mmap fixed failed, %v", mmap1Err)
		return
	}

	_, mmap2Err := mmap(vaddr+uintptr(size), size, syscall.MAP_SHARED|syscall.MAP_FIXED, fdptr)
	if mmap2Err != nil {
		err = fmt.Errorf("bytebuffers.Buffer: second mmap fixed failed, %v", mmap2Err)
		return
	}
...
func mmap(addr uintptr, length, flags int, fd uintptr) (uintptr, error) {
	...
	if err != 0 {
		return 0, os.NewSyscallError("mmap", err)
	}
